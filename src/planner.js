...
const geoapifyAPI =
  "https://api.geoapify.com/v1/geocode/search?text={search}&apiKey=4db075d5fc424f58a614eb428bd951b7";
const nsAPIKey = "a0595411f0ee485c8b291e973d18bca2";
const nsStationURL =
  "https://gateway.apiportal.ns.nl/nsapp-stations/v2/nearest?lat={lat}&lng={lng}&limit=1&includeNonPlannableStations=false";
const nsReisInfoURL =
  "https://gateway.apiportal.ns.nl/reisinformatie-api/api/v3/trips?fromStation={fromStation}&toStation=UT&dateTime={dateTime}&searchForArrival=true&addChangeTime=5"; // [&viaStation]
...
async function main() {
  try {
    ...
    const chosenNumber = await getUserInput(
      "Kies het getal van het resultaat waar je wilt vertrekken:"
    );
...
async function getLocations(input) {
  return new Promise(async (resolve) => {
    try {
      let apiSearch = geoapifyAPI.replace(
        "{search}",
